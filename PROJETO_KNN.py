...
b_treino = [[66707599984, 'Conservador', (5100., 3500., 1400., 200.)],
...
b_teste =  [[45926320819, '', (5800., 4000., 1200., 200.)],
...
def dist_euclidiana(treino, teste):
  ...
  for i in range(len(treino)):
    soma+=(treino[i]-teste[i])**2
  return soma**0.5

#votacao vizinhos 
def votacao(vizinhos):
  contagem_classes = {}
  for ponto in distancias[:k]:
    classe = ponto[1]
    if classe in contagem_classes:
      contagem_classes[classe] += 1
    else:
      contagem_classes[classe] = 1
  return max(contagem_classes)

#Inicio do codigo
resultado_teste=[] 
#Calculei o log de 120 que me deu aprozimadamente 2, porem escolhi o proximo numero impar para desempate 
k=3
for p_teste in b_teste:
    distancias=[]
    for p_treino in b_treino:
        distancia = dist_euclidiana(p_treino[2], p_teste[2])
        distancias.append((distancia, p_treino[1]))

    distancias.sort(key=lambda x: x[0])
    vizinhos_proximos=distancias[:k]
    classificacao = votacao(vizinhos_proximos)
    resultado_teste.append([p_teste[0], classificacao, p_teste[2]])
